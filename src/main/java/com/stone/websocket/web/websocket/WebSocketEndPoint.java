	...
	protected void handleTextMessage(WebSocketSession session,
		...
		try {
			if (ProtocolUtil._CUSTOMER_ONLINE.equals(protocolHeader)) {// 客服上线
				...
				ProtocolUtil._LEISURE_CUSTOMER_MAP.put(customerId, session);
				...
				ProtocolUtil._LEISURE_CUSTOMER_MAP.remove(customerId);
				if (null != customerId) {
					// 用户对应客服
					ProtocolUtil._USER_CUSTOMER_Relation.put(userId, customerId);
					
					WebSocketSession customerSession = ProtocolUtil._CUSTOMER_MAP
							.get(customerId);
					TextMessage userOnlineMessageOne = new TextMessage(
							ProtocolUtil._USER_ONLINE + ProtocolUtil._SEPARATOR +userId);
					customerSession.sendMessage(userOnlineMessageOne);
					TextMessage userOnlineMessageTwo = new TextMessage(
							ProtocolUtil._SYS_PROMPT + ProtocolUtil._SEPARATOR +customerId);
					session.sendMessage(userOnlineMessageTwo);
					System.out.println("用户："+userId+"与客服："+customerId+"已建立连接");
				} else {
					TextMessage userOnlineMessage = new TextMessage(
	...
	public void afterConnectionClosed(WebSocketSession session,
		...
		if (userOffline) {// 用户掉线
			...
			if (null != customerSession) {// 发送用户离线消息给客服
				TextMessage userOffLineMsg = new TextMessage(ProtocolUtil._USER_OFFLINE + ProtocolUtil._SEPARATOR+ id);
				customerSession.sendMessage(userOffLineMsg);
				// 移除用户和客服关系
				String cistomerId = ProtocolUtil._USER_CUSTOMER_Relation.get(id);
				ProtocolUtil._LEISURE_CUSTOMER_MAP.put(cistomerId, ProtocolUtil._CUSTOMER_MAP.get(cistomerId));
				ProtocolUtil._USER_CUSTOMER_Relation.remove(id);
				System.out.println("用户："+id+"与客服："+cistomerId+"已断开连接");
			}
		} else {// 客服掉线
			ProtocolUtil._CUSTOMER_MAP.remove(id);
			System.out.println("=====客服：" + id + "下线了=====");
			String key = null;
	         for(String getKey: ProtocolUtil._USER_CUSTOMER_Relation.keySet()){
		             if(ProtocolUtil._USER_CUSTOMER_Relation.get(getKey).equals(id)){
		                 key = getKey;
		             }
		         }
	        if(key == null) {
	        	ProtocolUtil._LEISURE_CUSTOMER_MAP.remove(id);
	        }else {
				WebSocketSession userSession = ProtocolUtil._USER_MAP.get(key);
				ProtocolUtil._LEISURE_CUSTOMER_MAP.remove(id);
				if (null != userSession) {
					ProtocolUtil._USER_CUSTOMER_Relation.remove(id);
					System.out.println("用户："+key+"与客服："+id+"已断开连接");
					TextMessage userOffLineMsg = new TextMessage(ProtocolUtil._CUSTOMER_OFFLINE + ProtocolUtil._SEPARATOR+ id);
					userSession.sendMessage(userOffLineMsg);
				}
	        }
		}
		// 移除IP和用户关系
...
/*		if (ProtocolUtil._CUSTOMER_MAP.keySet().size()==0) {
			System.out.println("暂时没有客服在线！！！");
		}else {
			System.out.println("当前有"+ProtocolUtil._CUSTOMER_MAP.keySet().size()+"名客服在线中！！！");
			System.out.println("非空闲客服ID"+ProtocolUtil._USER_CUSTOMER_Relation.values());
			
		}*/
		
		for (String customerKey : ProtocolUtil._LEISURE_CUSTOMER_MAP.keySet()) {
